  ...
  for (let dbGamesChunk of gamesChunked) {
    await Promise.all(
      dbGamesChunk.map(async dbGame => {
        ...
        if (
          msfGame?.schedule.playedStatus === types_1.MSFGamePlayedStatus.COMPLETED &&
          homeScore !== null &&
          awayScore !== null
        ) {
          const winner =
            homeScore === awayScore ? null : homeScore > awayScore ? dbGame.home : dbGame.away;
...
function getRecord(dbGames, currGame, teamId) {
  const prevDoneGamesWithTeam = dbGames.filter(
    g => (g.home === teamId || g.away === teamId) && g.week < currGame.week && g.done
  );
